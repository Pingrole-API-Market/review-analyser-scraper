...
class GoogleScraper:
    ...
    async def _scrape_page(
        ...
        try:
            ...
            logger.warning("[google] Reviews tab not found, trying alternate selector")
            try:
        ...
        try:
            ...
            if await sort_btn.count() > 0:
                ...
                newest_option = page.locator("li:has-text('Newest'), div[data-index='1']").first
                if await newest_option.count() > 0:
                    await newest_option.click()
                    await random_delay(1500, 2500)
        except Exception:
            logger.debug("[google] Could not sort by newest")
...
def _parse_relative_date(raw: str) -> str:
    ...
    if not m:
        # Try to detect absolute date patterns
    ...
    delta = timedelta(days=qty * delta_map.get(unit, 1))
