...
class YelpScraper:
    ...
    async def _scrape_all(
        ...
        try:
            first_link = page.locator(
                "h3 a[href*='/biz/'], a[class*='businessName']"
            ).first
        ...
        while len(reviews) < limit:
            ...
            try:
                read_more_btns = await page.locator("button:has-text('Read more')").all()
            ...
            if not cards:
                # Try JSON-LD scrape as fallback
            ...
            # Yelp paginates in steps of 10 or 20
    ...
    async def _json_ld_reviews(self, page: Page) -> list[dict]:
        """Extract reviews from JSON-LD structured data as fallback."""
        import json
        reviews = []
        try:
